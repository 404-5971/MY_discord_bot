...
async def ttt(ctx, *, args):
    ...
    if opponent_username == "Eli Bot":
        thread = await ctx.channel.create_thread(name='Tic Tac Toe', type=discord.ChannelType.public_thread)
        await ctx.send("You challenged Eli Bot! Let's play.")

        # Send initial message with the board
        await thread.send('To play this game type the number of the space you want to move. You have 30 seconds to make a move before the game is cancelled.')
        game_message = await thread.send(f"{sender_username}, it's your turn.\n```\n{display_board(board)}\n```")
